...
- [x] **Deployment/build process**: the app was dev-mode-only (hardcoded `localhost` on both sides, `adapter-auto` which can't produce a runnable server on its own). Backend now reads `CORS_ORIGINS` from the environment (comma-separated, defaults to the dev server's origin) instead of a hardcoded allow-list. Frontend's `API_BASE` now reads `VITE_API_BASE` at build time, falling back to `localhost:8010` for zero-config `npm run dev`. Switched the SvelteKit adapter from `adapter-auto` (picks a platform adapter automatically but produces nothing runnable when it can't detect one) to `@sveltejs/adapter-node`, so `npm run build` now always produces a self-hostable `build/` directory. Verified end-to-end, not just "the build didn't error": ran `npm run build` then `node build/index.js` and confirmed both `/` and `/methodology` return real rendered HTML with correctly linked asset bundles. Full deploy steps in [README.md](../README.md#deploying).
